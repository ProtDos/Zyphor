...
if __name__ == "__main__":
    ...
    # User input for Alice's initial message
    message_to_bob = input(Fore.BLUE + "Enter a message for Alice to send to Bob: " + Style.RESET_ALL)

    # Initial Message Exchange
    alice.send_message(server, bob, message_to_bob)

    print("-"*20)

    # Bob retrieves and processes the message
    messages_for_bob = server.retrieve_messages(bob)
    if messages_for_bob:
        sender, sealed_message = messages_for_bob[0]
        received_message = bob.receive_message(server, alice, sealed_message)
        if received_message:
            print(Fore.CYAN + f"Bob received: {received_message}" + Style.RESET_ALL)

    # Response Setup (Mailbox Rotation)
    bob.rotate_mailbox(server, alice)

    # Alice retrieves the notification and updates Bob's mailbox
    messages_for_alice = server.retrieve_messages(alice)
    if messages_for_alice:
        sender, sealed_notification = messages_for_alice[0]
        notification = alice.unseal_message(sealed_notification)
        new_mailbox_id, signature, signature_key_hex = notification.split("|")
        signature_key = bytes.fromhex(signature_key_hex)
        if alice.verify_mailbox_id_signature(new_mailbox_id, signature, signature_key):
            bob.mailbox = new_mailbox_id
            print(Fore.MAGENTA + f"Alice updated Bob's mailbox to: {bob.mailbox}" + Style.RESET_ALL)
        else:
            print(Fore.RED + "Alice failed to verify Bob's new mailbox ID" + Style.RESET_ALL)

    # User input for Bob's response
    message_to_alice = input(Fore.BLUE + "Enter a message for Bob to send to Alice: " + Style.RESET_ALL)

    # Ongoing Communication (Simplified)
    bob.send_message(server, alice, message_to_alice)

    messages_for_alice = server.retrieve_messages(alice)
    if messages_for_alice:
        sender, sealed_message = messages_for_alice[0]
        received_message = alice.receive_message(server, bob, sealed_message)
        if received_message:
            print(Fore.CYAN + f"Alice received: {received_message}" + Style.RESET_ALL)