...
7. [Comparison with Other Messaging Apps](#comparison-with-other-messaging-apps)
...
-   **Dummy traffic**: This obscures the timing of messages or key exchanges and further reduces metadata for adversaries. Adding **Decoy Rotations** is also a great way to obscure real rotations.
...
<details>
  <summary>Show protocol</summary>

![protocol](assets/full.svg "Full Protocol")
...
<details>
  ...
  ![image](assets/sealed.png "Sealed Messaging Protocol")
...
-   **Attacks**: DDoS attacks could large slow the traffic of a server down. Thats why it is important to have a strong firewall in place. Systems like the blinding of keys for mailbox creation help limit attacks.
...
#### Why no decentralization

I have decided not to use a decentralized system like [Session](https://getsession.org/) does for the following reasons:

-   **Simplicity**: A centralized system can be simpler to design, implement, and maintain compared to a decentralized one. It may require less complex infrastructure and coordination between nodes.

-   **Scalability**: Centralized systems can often scale more easily since there is a single point of control. In a decentralized system, managing the coordination and communication between multiple nodes can be more challenging as the system grows.

-   **Reliability**: Centralized systems offer a higher level of reliability compared to decentralized systems. With a centralized approach, the system's functionality is not dependent on the availability and reliability of multiple nodes. In contrast, decentralized systems can be more susceptible to failures or unavailability of individual nodes, which can impact the overall system's performance. Considering that this protocol and the associated app are still in their early stages, establishing a stable decentralized network would require the participation of multiple users, which cannot be guaranteed initially.
